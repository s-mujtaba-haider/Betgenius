...
Written **before** any round-4 number existed. Every acceptance criterion below
...
## Standing rules (unchanged from rounds 2 and 3)
...
`_c60`, tag `c60-round2`. 4 PASS / 5 PASS_WITH_RESTRICTIONS / 1 VETO / 1 FAIL;
full-OOS gate 9/11; verdict gate 4/11; 0 of 16,982 board rows unservable.
Physical copy in `checkpoint_r4_baseline/`, source and cache hashes recorded
there.

## What round 3 left open

Round 3's most promising result was the price-anchored architecture (E21):
...
floor is not a comparable setting across the two arms.** Round 4 opens here.
...
* **free** — each arm searches the whole grid on SELECT-A and is scored on
  SELECT-B. This reproduces round 3's comparison.
...
round-3 recommendation to revisit it is withdrawn with evidence.
...
Round 4 runs E22 onward. **No experiment may be re-run with a changed
criterion after seeing its result**, and no result may be taken from a re-cut of
the same held-out tail. If every experiment fails, the `_c60` solution stands and
round 4 reports that, exactly as round 3 did.
...
production-compatible pipeline. If the honest answer is below 9/11, round 4 says
...
This is the same misalignment round 3 identified inside the model search — *"the
...
and the round-1 failure mode cannot recur.
...
per-market layer is rejected and `_c60` stands.
