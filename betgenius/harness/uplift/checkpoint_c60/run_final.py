...
harness's own published verdicts bit for bit.
    ...
    python harness/uplift/run_final.py --conf=60 [--lag=N] [--price=best] [--tag=NAME]

`--conf=60` is the setting the headline table is produced at, and it is the one
to use. It pins the confidence floor to the value production already enforces --
isEvPassPick returns false below 60 -- and takes that knob OUT of the global
search. Without it the search may choose a floor of 0 or 55, and a board built
there is made of picks the product would never display: audited with
parity_audit.py, the swept board is 36% such rows and its `totals` board is 90%.
Experiment E18 is the held-out test that says pinning also generalises better.
"""
import itertools
import json
import os
...
def choose_global(caches, cuts, confs=CONFS):
...
def main():
    ...
    blocks = int(next((a.split("=")[1] for a in flags if a.startswith("--blocks=")),
                      policy.N_BLOCKS))
    # --conf=N pins the confidence floor and takes it OUT of the global search.
    # 60 is the value production already enforces: isEvPassPick returns false
    # below it (harness/lib/metrics.ts:149), and the D-164 heavy-juice veto has
    # no branch that fires under it either, so a board built at a lower floor is
    # made of picks the veto cannot reach and the product would never show.
    # Pinning strictly shrinks the search; see experiment E18.
    confs = ([int(next(a.split("=")[1] for a in flags if a.startswith("--conf=")))]
             if any(a.startswith("--conf=") for a in flags) else list(CONFS))
    ...
    spec, tau, min_conf, collapse, sweep = choose_global(caches, cuts, confs)
