# Round 3+ — data expansion and final optimisation: the result
...
verified: candidate counts match the `_c60` cache exactly. An outcome that was
...
every condition is in `reports/overrides_r4v.csv`.
...
`OPTIMISATION_ROUND3_PLAN.md` Addendum 3 rather than presented as if the amended
...
Full ledger with hypotheses, criteria and results: `reports/experiment_log_r4.csv`.
...
| | `_c60` (incumbent) | `BASELINE_EXPANDED` | **FINAL** |
|---|---|---|---|
...
That is the single most important production finding in this round, and it points
...
7. The full-OOS gate count fell 9 → 7. Anyone comparing headline numbers across
   rounds must compare **both-gates** counts, not full-OOS counts.
...
# --- the member cache, including the round-3+ families ---
...
UPLIFT_DATA_DIR="$D" UPLIFT_CACHE_DIR="$C" python harness/uplift/run_final_r4.py --conf=60 --tag=_r4v
UPLIFT_DATA_DIR="$D" UPLIFT_CACHE_DIR="$C" python harness/uplift/parity_audit.py --tag=_r4v
UPLIFT_DATA_DIR="$D" UPLIFT_CACHE_DIR="$C" python harness/uplift/season_analysis.py --tag=_r4v
...
UPLIFT_DATA_DIR="$D" UPLIFT_CACHE_DIR="$C" python harness/uplift/run_final_r4.py --conf=60 --tag=_e33 --pairs

# --- the incumbent, unchanged and still reproducing ---
python harness/uplift/run_final.py --conf=60 --tag=_c60
```

`_c60` reproduces **bit-identically** from the current source: caches rebuilt with
the modified `frames.py` and `policy.py` match member for member, and
`final_c60.csv` is identical on every column.
...
2. **Fix the entry-snapshot convention.** The warehouse switched from 6 h to 1 h
   ...
   should match what production actually prices at. This round showed the choice
