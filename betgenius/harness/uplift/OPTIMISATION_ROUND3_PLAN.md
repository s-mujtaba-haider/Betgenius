# Round 3+ — data expansion and final optimisation: the plan
...
> Naming note. `OPTIMISATION_ROUND2.md` and `OPTIMISATION_ROUND3.md` already
> exist and are untouched. This file and `OPTIMISATION_ROUND3_FINAL.md` are the
...
E23 did produce the most important measurement of the round, and it is carried
...
**That is why this round leads with data rather than with models.** The evidence
...
is unchanged by construction — verified: candidate counts match the `_c60` cache
...
| name | data | purpose |
|---|---|---|
| `_c60` | current | the frozen incumbent, unchanged |
...
**Hypothesis.** `BASELINE_EXPANDED` beats `_c60` on markets clearing both gates.
...
**Rollback.** If the expanded data reduces the PASS count, `_c60` stands as the
...
`PASS_WITH_RESTRICTIONS`. If the honest result is below that, round 3+ reports
