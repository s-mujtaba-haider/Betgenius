// D-825 — Point-in-time leak-safe backfill of factors with snapshot history
// (xwoba, launch_angle, sweet_spot, hard_hit) on the resolved batter_hits
// cohort. Mirrors D-801/D-821b TB pattern. Imports scoreBatterHits.
//
// CONTACT-RATE EXCLUDED: D-824 contact-rate cache has only 1 snapshot day
// (built 2026-06-29) — NOT backfillable. Forward retune (D-824b+) tests it.
// This iteration tests the EXISTING factor stack at full n=9,184.
//
// IMPORTS the real scorer (scoreBatterHits) — does NOT re-implement
...
import { scoreBatterHits, type BatterScoringContext } from "../_shared/scoring_mlb_v2.ts";
...
function buildMinimalContext(
  ...
  // weather, no ballpark, etc.). For D-825's purpose we only extract
  // the 4 D-797 factor values from the result; the other fields are
  // discarded by the caller.
  return {
    batter: {
      fullName: "_d825_backfill",
      team: "_",
      opponentTeam: "_",
      isHome: true,
      gameTime: "",
...
Deno.serve(async (req) => {
  ...
  const picksUrl = `${SUPABASE_URL}/rest/v1/pick_history?select=id,player_id,player_name,game_date,pick_side,line,odds&mlb_market_type=eq.batter_hits&hit=not.is.null&voided=eq.false&game_date=gte.2026-05-22&game_date=lte.2026-06-28${xwobaFilter}&order=game_date.asc`;
  const picks = (await fetchAll<Pick>(picksUrl)).slice(offset);
  console.log(`[d825] pulled ${picks.length} HITS cohort picks`);
  ...
  console.log(`[d825] distinct player_ids: ${playerIds.length}`);
  ...
  console.log(`[d825] loaded xstats=${xstats.length} exitVelo=${exitVelo.length} snapshots`);
  ...
  for (const p of picksToProcess) {
    ...
    let result: ReturnType<typeof scoreBatterHits>;
    try {
      result = scoreBatterHits(ctx);
    } catch (e) {
      console.error(`[d825] scorer error pick_id=${p.id} err=${e}`);
      continue;
    }
    scored++;
