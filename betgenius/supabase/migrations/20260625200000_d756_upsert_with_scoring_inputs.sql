-- D-756 — Add scoring_inputs to the upsert_pick_history RPC.
--
-- HIDDEN BUG: D-742 PART 3 added the `scoring_inputs JSONB` column to
-- pick_history and the edge-function payload write, BUT NEVER added the
-- column to the RPC's INSERT VALUES list. The RPC's
-- `jsonb_populate_record(NULL::pick_history, payload)` does fill
-- `rec.scoring_inputs` from the payload (the column exists on the table type),
-- but the explicit `INSERT INTO pick_history (col1, col2, ...) VALUES (...)`
-- clause selects what actually gets written — and `scoring_inputs` was
-- omitted. Result: every pitcher_k pick since D-742 was supposed to write
-- scoring_inputs has it as NULL. The capture in the edge function looked
-- correct; the RPC silently dropped it at the SQL boundary. D-756's edge-
-- function extension to batter + game markets surfaced the same gap.
--
-- CHANGES vs D-723:
--   1. Add `scoring_inputs` to the INSERT column list.
--   2. Add `rec.scoring_inputs` to the VALUES tuple.
--   3. Add `scoring_inputs = COALESCE(EXCLUDED.scoring_inputs, pick_history.scoring_inputs)`
--      to DO UPDATE SET — so on-conflict UPSERTs preserve any prior captured
--      inputs if the new payload doesn't carry one (defense-in-depth).
--
-- NO column changes. NO schema changes. NO behavior changes to existing fields.
-- Adding scoring_inputs to INSERT/UPDATE is purely additive — a payload that
-- omits scoring_inputs will INSERT null (status quo) and UPDATE preserves
-- existing.
...
BEGIN
  ...
  INSERT INTO public.pick_history (
    ...
    breakdown,
    confidence_pre_cap,
    -- D-756 — capture-at-score-time parity snapshot
    scoring_inputs
  ) VALUES (
  ...
  DO UPDATE SET
    ...
    -- D-756 — preserve existing scoring_inputs if a later UPSERT omits it
