-- D-797 — Extend upsert_pick_history RPC INSERT + DO UPDATE SET to persist
-- the 5 newly-added factor columns (4 extra-base factors + babip non-return fix).
--
-- Companion to 20260628010000 (which adds the columns). Without this RPC
-- extension, the columns exist on pick_history but jsonb_populate_record drops
-- the values at the INSERT VALUES boundary (the D-742/D-758/D-785 silent-drop
-- bug class). Both layers needed for the columns to populate on fresh picks.
--
-- Pure additive. Every other field preserved byte-for-byte from D-785.
