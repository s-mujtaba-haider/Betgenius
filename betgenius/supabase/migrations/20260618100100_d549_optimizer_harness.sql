...
DECLARE
  ...
  _batch_tag        TEXT    := 'D-549-pitcher_k';
  _market_filter    TEXT    := 'pitcher_k';            -- mlb_market_type=
  _projection_key   TEXT    := 'projected_k';          -- breakdown key
  -- Factors to re-weight. Pre-filtered to the signal-carrying ones from
  -- D-549 SHIP 1 (|r|>0.20). Multipliers searched per factor.
...
BEGIN
  ...
  SELECT
    ...
    NULLIF(breakdown->>'projected_k','')::numeric            AS projection,
    -- per-factor: pull (key, weighted_score) into JSONB so coordinate
    -- descent can iterate without per-factor branching.
    jsonb_build_object(
      'score_pitcher_k_rate',
        CASE WHEN (breakdown->>'score_pitcher_k_rate') ~ '^-?[0-9.]+$'
             THEN (breakdown->>'score_pitcher_k_rate')::numeric END,
      'score_pitcher_form',
        CASE WHEN (breakdown->>'score_pitcher_form') ~ '^-?[0-9.]+$'
             THEN (breakdown->>'score_pitcher_form')::numeric END,
      'score_pitcher_xera_edge',
        CASE WHEN (breakdown->>'score_pitcher_xera_edge') ~ '^-?[0-9.]+$'
             THEN (breakdown->>'score_pitcher_xera_edge')::numeric END,
      'score_pitcher_baa',
        CASE WHEN (breakdown->>'score_pitcher_baa') ~ '^-?[0-9.]+$'
             THEN (breakdown->>'score_pitcher_baa')::numeric END,
      'score_opposing_lineup_k',
        CASE WHEN (breakdown->>'score_opposing_lineup_k') ~ '^-?[0-9.]+$'
             THEN (breakdown->>'score_opposing_lineup_k')::numeric END
    ) AS factors,
