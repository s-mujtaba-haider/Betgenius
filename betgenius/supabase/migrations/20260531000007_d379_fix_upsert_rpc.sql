-- D-379 SHIP 2 FIX — correct ON CONFLICT spec + full DO UPDATE SET preserving
-- the D-204 behavior. My initial 20260531000006 migration changed ON CONFLICT
-- to (player_name, prop_type, line, pick_side, game_time) which does not match
-- any unique constraint on pick_history. The actual constraint is
-- (player_name, prop_type, line, game_date) WHERE is_synthetic = false.
--
-- This migration regenerates upsert_pick_history with the correct constraint
-- and faithfully ports the D-204 DO UPDATE SET clauses, adding breakdown.
--
-- The breakdown column (added in 20260531000006) is preserved across this
-- migration since it's a CREATE OR REPLACE FUNCTION change only.
...
'D-379 SHIP 2 FIX — corrects ON CONFLICT spec from my initial 20260531000006
migration. Faithful port of D-204 DO UPDATE SET clauses + new breakdown column
both in INSERT VALUES and DO UPDATE SET (with COALESCE preserving prior).';
