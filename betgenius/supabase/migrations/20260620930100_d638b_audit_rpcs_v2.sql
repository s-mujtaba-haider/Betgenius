-- D-638b — RPC wrappers around the 4 D-638 audit queries.
-- ────────────────────────────────────────────────────────────────────
-- Lets the audit run via PostgREST /rpc/<name> with the local
-- service-role key. Each function:
--   * SECURITY DEFINER + search_path locked.
--   * Bumps statement_timeout LOCAL to 180s (the analytical queries
--     can scan large pick_history slices).
--   * Returns rows shaped to mirror the D-638 query columns exactly.
--   * NOTHING MUTATING — pure SELECT.
--
-- Rollback:
...
BEGIN
  ...
  SELECT
    ...
    END AS verdict
  FROM base
  GROUP BY base.prop_type
  ORDER BY real_ev_pp ASC;
END $$;
...
BEGIN
  ...
  RETURN QUERY
  WITH base AS (
    SELECT ph.id, ph.prop_type, ph.breakdown
    FROM public.pick_history ph
    WHERE ph.prop_type IN (
      'batter_hr','batter_rbis','batter_runs_scored',
      'batter_strikeouts','pitcher_outs','game_total'
  ...
  totals AS (
    SELECT base.prop_type, COUNT(*) AS n_total FROM base GROUP BY base.prop_type
  ),
  keys AS (
    SELECT b.prop_type, kv.key, kv.value
  ...
  FROM agg a JOIN totals t USING (prop_type)
  ...
  ORDER BY a.prop_type, pct_active ASC, a.factor_name;
END $$;
...
BEGIN
  ...
  per_factor AS (
    SELECT
      k.prop_type, k.key AS factor_name, t.n_total,
      SUM(CASE WHEN jsonb_typeof(k.value) = 'number'
               AND (k.value)::text::numeric <> 0 THEN 1 ELSE 0 END) AS n_active
    FROM keys k JOIN totals t USING (prop_type)
    GROUP BY k.prop_type, k.key, t.n_total
  ),
  flagged AS (
    SELECT
      per_factor.factor_name,
      per_factor.prop_type,
  ...
  SELECT
    ...
    STRING_AGG(flagged.prop_type, ', ' ORDER BY flagged.prop_type)
...
BEGIN
  ...
  FROM base
  GROUP BY base.prop_type, base.post_d630
  ORDER BY base.prop_type, base.post_d630;
END $$;
