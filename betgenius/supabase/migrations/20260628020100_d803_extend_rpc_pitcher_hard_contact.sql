-- D-803 — Extend upsert_pick_history RPC INSERT + DO UPDATE SET to persist
-- the score_pitcher_hard_contact_allowed column (matchup-side counterpart
-- to D-797 score_batter_hard_hit; closes D-800 finding on pitcher side).
--
-- Companion to 20260628020000 (which adds the column). Without this RPC
-- extension, jsonb_populate_record drops the value at INSERT VALUES
-- boundary (D-742/D-758/D-785 silent-drop class). Both layers needed.
--
-- Pure additive. Every other field preserved byte-for-byte from D-797.
...
BEGIN
  ...
  INSERT INTO public.pick_history (
    ...
    score_team_form,
    -- D-785 — 10 batter factor columns previously in breakdown JSONB only
    ...
    -- D-797 — babip non-return fix + 4 new extra-base factors
    score_batter_babip,
    ...
    rec.score_team_form,
    -- D-785 — 10 batter factor columns
    ...
    -- D-797 — babip + 4 new extra-base factors
