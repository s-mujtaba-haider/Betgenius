-- D-747-DEPLOY STEP 4 — FINAL RESTORE: bring weights back to D-746 baseline
-- after Test B has been captured. Verify exact restore via NOTICE assertion.
...
BEGIN
  ...
  RAISE NOTICE 'D-747 FINAL restore: mismatch_count=% (expected 0)', mismatch_count;
END $$;

-- Also re-rescore validate window with D-746 weights so the live data
-- matches the current production scoring (so the next downstream usage isn't
-- left with Test B's ×1.5 values).
