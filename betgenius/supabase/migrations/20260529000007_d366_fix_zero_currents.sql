-- D-366 SHIP 1 patch — wrap each rescale factor in COALESCE(..., 0) so that
-- weights currently at 0 (e.g. D-364's wind_direction_hr, pitcher_hr_per_9)
-- don't NULL-propagate through the whole confidence sum.
--
-- Mathematically correct: if current_w = 0, stored s_X = f_raw * 0 = 0, so
-- the term 0 * (new/curr - 1) should be 0 regardless of new.
-- Previously: NULLIF(0, 0) → NULL → division NULL → s_X * NULL = NULL → entire
-- new_conf was NULL, dropping the row from every COUNT FILTER.
--
-- Rollback: DROP FUNCTION IF EXISTS public.d366_score_at_weights(jsonb, jsonb, int);
--           then re-apply 20260529000006_d366_extended_mv.sql
