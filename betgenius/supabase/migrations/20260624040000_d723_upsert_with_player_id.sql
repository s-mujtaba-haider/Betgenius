-- D-723 — Add player_id to upsert_pick_history RPC. Closes the writeback gap
-- where cron-inserted picks lacked player_id after D-719's one-shot backfill,
-- forcing D-721/D-722/D-724 player_id-first readers to fall back to name match
-- on every new pick.
--
-- CHANGES vs D-446 (the current in-prod RPC):
--   1. Add `player_id` to the INSERT column list (it's already in the
--      pick_history schema per D-719; jsonb_populate_record fills `rec.player_id`
--      from payload, but the explicit INSERT list controlled what got written).
--   2. Add `rec.player_id` to the VALUES tuple.
--   3. Add `player_id = COALESCE(EXCLUDED.player_id, pick_history.player_id)` to
--      DO UPDATE SET — so on-conflict UPSERTs preserve any prior pid if the new
--      payload doesn't carry one (defense-in-depth).
--
-- This migration ONLY adds player_id. Every other column behavior is preserved
-- byte-for-byte from D-446 to keep blast radius minimal.
--
-- NO column changes. NO schema changes. NO behavior changes to existing fields.
-- Adding player_id to INSERT/UPDATE is purely additive — a payload that
-- omits player_id will INSERT null (status quo) and UPDATE preserves existing.
...
BEGIN
  ...
  DO UPDATE SET
    -- D-723: COALESCE so existing pid isn't clobbered if the new payload omits player_id
