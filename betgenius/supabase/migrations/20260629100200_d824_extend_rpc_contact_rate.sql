-- D-824 — Extend upsert_pick_history RPC INSERT + DO UPDATE SET to persist
-- 4 new columns: score_batter_contact_rate (hits-only factor) + 3 raw % columns
-- (batter_whiff_pct, batter_contact_pct, batter_z_contact_pct). Pure additive.
-- D-758 silent-drop guard scans this file for INSERT-column presence.
...
BEGIN
  ...
  INSERT INTO public.pick_history (
    ...
    -- D-824 — hits-only contact-rate factor + 3 raw % columns
    score_batter_contact_rate, batter_whiff_pct, batter_contact_pct, batter_z_contact_pct
  ) VALUES (
    ...
    -- D-824 — hits-only contact-rate factor + 3 raw % columns
    rec.score_batter_contact_rate, rec.batter_whiff_pct, rec.batter_contact_pct, rec.batter_z_contact_pct
  )
  ...
  DO UPDATE SET
    ...
    -- D-824 — hits-only contact-rate factor + 3 raw % columns
    score_batter_contact_rate = COALESCE(EXCLUDED.score_batter_contact_rate, pick_history.score_batter_contact_rate),
    batter_whiff_pct = COALESCE(EXCLUDED.batter_whiff_pct, pick_history.batter_whiff_pct),
    batter_contact_pct = COALESCE(EXCLUDED.batter_contact_pct, pick_history.batter_contact_pct),
    batter_z_contact_pct = COALESCE(EXCLUDED.batter_z_contact_pct, pick_history.batter_z_contact_pct)
