-- D-785 PART 1c — Extend upsert_pick_history RPC INSERT + DO UPDATE SET to
-- persist the 10 newly-added factor columns (the D-758 silent-drop fix).
--
-- The schema migration above adds 10 columns; this migration extends the RPC
-- so payloads with these top-level fields actually persist (rather than being
-- loaded into `rec` by jsonb_populate_record but dropped at the INSERT VALUES
-- boundary — the classic D-742 / D-758 silent-drop bug class).
--
-- D-785 EVALUATOR REQUIREMENT: "confirm 10 columns added to schema AND RPC
-- (not just schema — the D-758 trap was RPC omission)". Both layers are
-- needed for the columns to actually populate on fresh picks.
--
-- Pure additive. Every other field preserved byte-for-byte from D-758. The
-- 10 new columns: INSERT VALUES, COALESCE on DO UPDATE SET (so a missing
-- payload field doesn't NULL out an existing row's value).
