-- Phase 3b drift verification — same query as Phase 3a but against the
-- corrected backfill output (placement-bug fix in commit b2a7c42).
...
DECLARE
  ...
  cnt INT;
BEGIN
  SELECT id INTO phase3b_run FROM backfill_runs
  WHERE algorithm_version = '2026-05-11-phase3b-verify' LIMIT 1;
  RAISE NOTICE 'Phase 3b run_id: %', phase3b_run;

  IF phase3b_run IS NULL THEN
    RAISE NOTICE 'Phase 3b run not found — backfill may still be processing.';
    RETURN;
  END IF;

  SELECT COUNT(*) INTO cnt FROM pick_history WHERE backfill_run_id = phase3b_run;
  RAISE NOTICE 'Phase 3b synthetic rows: %', cnt;
  ...
  FOR r IN
    ...
    ORDER BY 1
  LOOP
    RAISE NOTICE '% mean=% std=% |Δ|>5: % |Δ|>10: % n=%',
      r.label, r.mean, r.std, r.gt5, r.gt10, r.n;
  END LOOP;

  RAISE NOTICE '';
  RAISE NOTICE '=== Per-factor mean abs drift (vs live) ===';
  FOR r IN
    SELECT 'stale_data       P2' AS f, ROUND(AVG(ABS(L_stale - P2_stale))::NUMERIC, 3) AS m, COUNT(*) FILTER (WHERE L_stale != P2_stale) AS nz FROM tw
    UNION ALL SELECT 'stale_data       P3b', ROUND(AVG(ABS(L_stale - P3_stale))::NUMERIC, 3), COUNT(*) FILTER (WHERE L_stale != P3_stale) FROM tw
    UNION ALL SELECT 'opp_defense      P2', ROUND(AVG(ABS(L_opp   - P2_opp  ))::NUMERIC, 3), COUNT(*) FILTER (WHERE L_opp   != P2_opp  ) FROM tw
    UNION ALL SELECT 'opp_defense      P3b', ROUND(AVG(ABS(L_opp  - P3_opp  ))::NUMERIC, 3), COUNT(*) FILTER (WHERE L_opp   != P3_opp  ) FROM tw
    UNION ALL SELECT 'trivial_pen      P2', ROUND(AVG(ABS(L_triv  - P2_triv ))::NUMERIC, 3), COUNT(*) FILTER (WHERE L_triv  != P2_triv ) FROM tw
    UNION ALL SELECT 'trivial_pen      P3b', ROUND(AVG(ABS(L_triv - P3_triv ))::NUMERIC, 3), COUNT(*) FILTER (WHERE L_triv  != P3_triv ) FROM tw
    UNION ALL SELECT 'rest             P2', ROUND(AVG(ABS(L_rest  - P2_rest ))::NUMERIC, 3), COUNT(*) FILTER (WHERE L_rest  != P2_rest ) FROM tw
    UNION ALL SELECT 'rest             P3b', ROUND(AVG(ABS(L_rest - P3_rest ))::NUMERIC, 3), COUNT(*) FILTER (WHERE L_rest  != P3_rest ) FROM tw
    UNION ALL SELECT 'b2b              P2', ROUND(AVG(ABS(L_b2b   - P2_b2b  ))::NUMERIC, 3), COUNT(*) FILTER (WHERE L_b2b   != P2_b2b  ) FROM tw
    UNION ALL SELECT 'b2b              P3b', ROUND(AVG(ABS(L_b2b  - P3_b2b  ))::NUMERIC, 3), COUNT(*) FILTER (WHERE L_b2b   != P3_b2b  ) FROM tw
    UNION ALL SELECT 'player_injury    P2', ROUND(AVG(ABS(L_inj   - P2_inj  ))::NUMERIC, 3), COUNT(*) FILTER (WHERE L_inj   != P2_inj  ) FROM tw
    UNION ALL SELECT 'player_injury    P3b', ROUND(AVG(ABS(L_inj  - P3_inj  ))::NUMERIC, 3), COUNT(*) FILTER (WHERE L_inj   != P3_inj  ) FROM tw
    UNION ALL SELECT 'usg_rate         P2', ROUND(AVG(ABS(L_usg   - P2_usg  ))::NUMERIC, 3), COUNT(*) FILTER (WHERE L_usg   != P2_usg  ) FROM tw
    UNION ALL SELECT 'usg_rate         P3b', ROUND(AVG(ABS(L_usg  - P3_usg  ))::NUMERIC, 3), COUNT(*) FILTER (WHERE L_usg   != P3_usg  ) FROM tw
    UNION ALL SELECT 'minutes_volume   P2', ROUND(AVG(ABS(L_mv    - P2_mv   ))::NUMERIC, 3), COUNT(*) FILTER (WHERE L_mv    != P2_mv   ) FROM tw
    UNION ALL SELECT 'minutes_volume   P3b', ROUND(AVG(ABS(L_mv   - P3_mv   ))::NUMERIC, 3), COUNT(*) FILTER (WHERE L_mv    != P3_mv   ) FROM tw
    UNION ALL SELECT 'minutes_stab     P2', ROUND(AVG(ABS(L_ms    - P2_ms   ))::NUMERIC, 3), COUNT(*) FILTER (WHERE L_ms    != P2_ms   ) FROM tw
    UNION ALL SELECT 'minutes_stab     P3b', ROUND(AVG(ABS(L_ms   - P3_ms   ))::NUMERIC, 3), COUNT(*) FILTER (WHERE L_ms    != P3_ms   ) FROM tw
    UNION ALL SELECT 'pace             P2', ROUND(AVG(ABS(L_pace  - P2_pace ))::NUMERIC, 3), COUNT(*) FILTER (WHERE L_pace  != P2_pace ) FROM tw
    UNION ALL SELECT 'pace             P3b', ROUND(AVG(ABS(L_pace - P3_pace ))::NUMERIC, 3), COUNT(*) FILTER (WHERE L_pace  != P3_pace ) FROM tw
    UNION ALL SELECT 'home_away        P2', ROUND(AVG(ABS(L_ha    - P2_ha   ))::NUMERIC, 3), COUNT(*) FILTER (WHERE L_ha    != P2_ha   ) FROM tw
    UNION ALL SELECT 'home_away        P3b', ROUND(AVG(ABS(L_ha   - P3_ha   ))::NUMERIC, 3), COUNT(*) FILTER (WHERE L_ha    != P3_ha   ) FROM tw
  LOOP
  ...
  FOR r IN
    SELECT 'live' AS label, COUNT(*) AS n,
    ...
    UNION ALL SELECT 'P2 synthetic', COUNT(*), SUM(CASE WHEN hit THEN 1 ELSE 0 END),
      ROUND(100.0 * SUM(CASE WHEN hit THEN 1 ELSE 0 END)
        / NULLIF(SUM(CASE WHEN hit IS NOT NULL THEN 1 ELSE 0 END), 0), 2)
    FROM pick_history WHERE backfill_run_id = phase2_run
      AND game_date = '2026-04-25'::DATE AND prop_type NOT IN ('spread','game_total')
      AND confidence >= 70 AND confidence < 80
    UNION ALL SELECT 'P3b synthetic', COUNT(*), SUM(CASE WHEN hit THEN 1 ELSE 0 END),
      ROUND(100.0 * SUM(CASE WHEN hit THEN 1 ELSE 0 END)
        / NULLIF(SUM(CASE WHEN hit IS NOT NULL THEN 1 ELSE 0 END), 0), 2)
    FROM pick_history WHERE backfill_run_id = phase3b_run
      AND game_date = '2026-04-25'::DATE AND prop_type NOT IN ('spread','game_total')
      AND confidence >= 70 AND confidence < 80
  LOOP
    RAISE NOTICE '% n=% hits=% wr=%', RPAD(r.label, 16), r.n, r.hits, r.wr;
  END LOOP;
END $$;
