-- D-204 — regenerate upsert_pick_history RPC with all 14 MLB columns.
--
-- §1.17 audit on the RPC writer path. Both INSERT VALUES and DO UPDATE SET
-- updated. NULLable columns; callers omitting MLB fields write NULL
-- (correct for NBA writers — they don't set MLB-specific values).
--
-- Rollback: re-apply 20260517000008_d199_upsert_pick_history_with_ai_verdict.sql.
...
BEGIN
  ...
  INSERT INTO public.pick_history (
    ...
    -- D-204 MLB columns (all NULLable; NBA writers omit, MLB writers populate)
    ...
    rec.mlb_market_type,
    COALESCE(rec.is_mlb_beta, true),  -- default true for any MLB row
  ...
  DO UPDATE SET
    ...
    -- D-204 MLB columns — COALESCE preserves prior value on caller omission
...
COMMENT ON FUNCTION public.upsert_pick_history(JSONB) IS
  'D-204 (May 17 2026): extends D-199 with 14 MLB factor + metadata columns (score_pitcher_k_rate, score_pitcher_form, ..., mlb_market_type, is_mlb_beta, mlb_beta_resolved_picks). All NULLable; NBA writers omit MLB fields, MLB writers populate. COALESCE on UPDATE preserves prior MLB values when caller omits.';
