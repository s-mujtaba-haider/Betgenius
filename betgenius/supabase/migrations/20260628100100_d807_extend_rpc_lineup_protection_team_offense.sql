-- D-807 — Extend upsert_pick_history RPC INSERT + DO UPDATE SET to persist
-- 2 new runs-only factor columns:
--   score_batter_lineup_protection (avg OPS of next 2 hitters behind)
--   score_batter_team_offense (batter team OPS season)
--
-- Companion to 20260628100000 (which adds the columns). Without this RPC
-- extension, jsonb_populate_record drops the values at INSERT VALUES
-- boundary (D-742/D-758/D-785/D-803 silent-drop class).
--
-- Pure additive. Every other field preserved byte-for-byte from D-803.
...
BEGIN
  ...
  INSERT INTO public.pick_history (
    ...
    -- D-807 — 2 new runs-only factor columns
