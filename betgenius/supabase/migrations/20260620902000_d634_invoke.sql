DO $$ DECLARE v_token text; v_req bigint; r RECORD; BEGIN
  ...
  RAISE NOTICE 'D-634 first run request_id=%', v_req;
  PERFORM pg_sleep(20);
  FOR r IN
    SELECT status_code, regexp_replace(LEFT(COALESCE(content::text,''),900), E'[\\n\\r]+', ' ', 'g') AS body
      FROM net._http_response WHERE id = v_req
  LOOP RAISE NOTICE 'resp status=% body=%', r.status_code, r.body; END LOOP;
  PERFORM pg_sleep(5);
  FOR r IN
    SELECT count(*) AS n, count(DISTINCT bookmaker) AS books,
     ...
     WHERE sport='mlb' AND game_date=to_char((now() AT TIME ZONE 'America/New_York')::date,'YYYY-MM-DD')
  LOOP
