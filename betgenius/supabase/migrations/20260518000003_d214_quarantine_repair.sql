-- D-214 Fix 5 repair — re-apply quarantine flag after rescore wiped it.
--
-- Root cause: upsert_pick_history INSERT VALUES used
-- COALESCE(rec.is_d214_quarantined, FALSE) which coerces NULL → FALSE
-- before the DO UPDATE SET COALESCE fallback runs. When process-games-mlb
-- omitted the field (rec.is_d214_quarantined IS NULL), EXCLUDED then
-- evaluated to FALSE (not NULL), and the UPDATE clause's COALESCE chose
-- FALSE over the existing TRUE.
--
-- Two-part fix:
--   1. UPDATE the 160 originally-quarantined picks back to TRUE.
--   2. Regen the RPC so future rescores preserve quarantine state.

BEGIN;

-- Part 1: re-apply quarantine flag + preserve confidence_pre_d214_fix
-- (which was also wiped on rescore for the same COALESCE-on-INSERT bug).
-- Identify the original-rescore cohort: today's MLB Beta picks with
-- recently-updated rows that have NULL confidence_pre_d214_fix (the bug
-- wiped them).
UPDATE public.pick_history
SET
  is_d214_quarantined = TRUE,
  quarantine_reason = COALESCE(quarantine_reason, 'D-214 cache infrastructure failure — picks generated against degraded MLB factor data 2026-05-17'),
  -- For picks rescored already (post Fix 5 first apply), confidence_pre_d214_fix
  -- was wiped. We can't recover the exact pre-fix value (already overwritten),
  -- so leave NULL for already-wiped picks. Only set it for newly-discovered
  -- quarantine cohort (none expected at this point).
  confidence_pre_d214_fix = COALESCE(confidence_pre_d214_fix, confidence)
WHERE sport = 'mlb'
  AND source = 'process-games-mlb'
  AND game_date = '20260517'
  AND is_mlb_beta = TRUE;

-- Part 2: regen RPC so INSERT VALUES preserves NULL for quarantine fields.
-- When process-games-mlb omits the fields, EXCLUDED is NULL, and DO UPDATE
-- COALESCE correctly falls through to preserve existing pick_history value.
-- For brand-new picks, the column DEFAULT FALSE on is_d214_quarantined
-- still kicks in via PostgREST when not in payload (jsonb_populate_record
-- leaves NULL → DB defaults applied at INSERT time IF the column is
-- omitted from the column list — but since we list it explicitly, NULL
-- gets inserted. The downstream impact is that brand-new NBA picks would
-- now have is_d214_quarantined=NULL rather than FALSE. Updating queries
-- to use `IS NOT TRUE` instead of `=FALSE` handles this gracefully.
...
BEGIN
  ...
  INSERT INTO public.pick_history (
    ...
    -- D-214 Fix 5 repair — pass NULL directly. Column DEFAULT FALSE on
    -- is_d214_quarantined still applies if rec.is_d214_quarantined is NULL
    -- AND nothing references the column DEFAULT (it doesn't — we pass NULL
    -- explicitly here). For brand-new picks omitting quarantine fields,
    -- they'll be NULL — downstream code should treat NULL as "not quarantined".
    rec.confidence_pre_d214_fix,
    rec.is_d214_quarantined,
...
BEGIN
  SELECT COUNT(*) INTO q_count FROM public.pick_history
    WHERE is_d214_quarantined = TRUE AND sport='mlb' AND game_date='20260517';
  RAISE NOTICE 'D-214 Fix 5 repair VERIFY: % MLB 20260517 picks marked quarantined', q_count;
END $$;
