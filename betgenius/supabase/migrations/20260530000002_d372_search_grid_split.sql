-- D-372 SHIP 1c — batched grid-search SQL function with split support.
-- Mirrors d366_search_weight_grid but adds p_split_mode and per_market output
-- to feed the classifier's market-regression check at n≥25.
--
-- Rollback: DROP FUNCTION public.d372_search_weight_grid(text, jsonb, jsonb, numeric[], int, text);

CREATE OR REPLACE FUNCTION public.d372_search_weight_grid(
  p_weight_col TEXT,
...
BEGIN
    ...
    SELECT
      ...
      fs.confidence
        ...
        + CASE WHEN fs.prop_type IN ('game_side', 'game_total') THEN
          ...
          ELSE 0 END
        AS new_conf
    FROM trials t CROSS JOIN base fs
  ),
  agg AS (
    SELECT
      gv,
  ...
  per_market AS (
    SELECT
      ...
      COUNT(*) FILTER (WHERE new_conf >= p_min_conf AND hit IS TRUE) AS hits
    FROM scored
    GROUP BY gv, prop_type
  ),
  per_market_per_grid AS (
    SELECT gv, jsonb_object_agg(prop_type, jsonb_build_object('n', n, 'hits', hits, 'hr', CASE WHEN n > 0 THEN hits::numeric / n ELSE 0 END)) AS pm
    FROM per_market GROUP BY gv
  ),
  combined AS (
    SELECT a.gv,
      jsonb_build_object(
        'objective_n', a.n,
        'objective_hits', a.hits,
        'objective_hit_rate', CASE WHEN a.n > 0 THEN a.hits::numeric / a.n ELSE 0 END,
        'per_market', pm.pm
      ) AS j
    FROM agg a LEFT JOIN per_market_per_grid pm USING (gv)
  )
  SELECT jsonb_object_agg(gv::text, j) INTO v_result FROM combined;
  RETURN v_result;
END $$;

GRANT EXECUTE ON FUNCTION public.d372_search_weight_grid(text, jsonb, jsonb, numeric[], int, text) TO service_role, authenticated;
