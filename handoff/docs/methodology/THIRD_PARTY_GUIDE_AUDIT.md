...
Written before any of the work it reports on had produced a number.
...
|---|---|---|
| Local branches | `git branch -a` | Five branches, all belonging to this project's own development. Neither guide branch name present. |
| Remote | `git remote -v` -> `<private internal repository>` | Same five. |
| Deleted-branch recovery | `git reflog --all`, `.git/packed-refs`, `git stash list` | 30 reflog entries, all accounted for by this project's own development. No packed refs. No stashes. No dangling commit mentions either name. |
| Whole object store | `git log --all --oneline` | All commits accounted for on the known branches. |
| Other repositories available | repository listing | Two other MLB repositories exist. Neither guide branch name present in either. |
| Their branch lists | branch listing | One branch each. Neither guide branch name present. |
| Their file trees | recursive tree listing | Neither contains `MLB Markets/`, `db/mlb_markets.duckdb`, `HANDOFF.md`, `POOLED_ML_STATUS.md`, `reports/MILESTONE_1_GATE_REPORT.md`, or any of the named scripts. Different codebases (`src/mlbgate`, `src/mlbedge`). |
| Whole disk | PowerShell recursive search of `<user-home>` (and `C:\Projects`, `C:\dev`, `C:\work`, `D:\` where they exist) for `mlb_markets.duckdb`, `POOLED_ML_STATUS.md`, `MILESTONE_1_GATE_REPORT.md`, `xgboost_pooled_A_factors.py`, `test_backfilled_markets.py`, `multi_model_comparison.py` | **Zero hits.** |
...
executed — and is, in the rest of later work — is the part that actually matters:
...
later work is built on. Each row was checked against the live source.
...
| Model: **Poisson / negative binomial count models** | **ALREADY PRESENT** | `features.py` builds both per prop market; the final configuration's E27 tested offering them as ensemble members directly |
| Model: **XGBoost** | **GENUINELY NEW** | not importable before later work; installed 2026-09-17 (xgboost 3.4.1) |
| Model: **LightGBM** | **GENUINELY NEW** | not importable before later work; installed 2026-09-17 (lightgbm 4.7.0) |
| Model: **FLAML AutoML** | **NEW, and rejected without testing** | An AutoML time budget searches model families and hyperparameters against a single objective on a single split. That is precisely the multiple-comparisons exposure the final configuration's guards exist to stop, and it cannot be made to honour the nested SELECT-A/SELECT-B contract without rewriting it into something that is no longer FLAML. Documented, not run |
...
is what later work's pre-registration is scoped to.
