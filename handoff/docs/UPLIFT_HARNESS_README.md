...
parity_audit.py    hands every row of a final board back to the SHIPPED
                   ...
                   any board: an early board fails it at 36%
...
python harness/uplift/run_final_model.py --conf=60 --tag=_v1   # THE run
python harness/uplift/parity_audit.py --tag=_v1          # must print 0 rejected
python harness/uplift/robustness.py --conf=60
python harness/uplift/final_matrix.py --tag=_v1          # THE table
python harness/uplift/build_workbook.py
```

The two SELECT-only experiments behind the key architectural decisions, neither of which
reads the verdict window:

```bash
python harness/uplift/exp_price_members.py    # E16
python harness/uplift/exp_nested_rules.py     # E17 / E18
python harness/uplift/compare_runs.py --a= --b=the earlier baseline
```

`docs/VALIDATION_REPORT.md` is the write-up: what was adopted, what was rejected,
